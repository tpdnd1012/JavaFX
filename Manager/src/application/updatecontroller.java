...
public class updatecontroller implements Initializable {
	...
	public static String updatetel = null;
	...
	// 업데이트 버튼 눌렀을때
	public void bupdateAction(ActionEvent e) {

		ObservableList<wmember> wmemberselect;

		wmemberselect = wmemberlist.getSelectionModel().getSelectedItems();

		updatetel = wmemberselect.get(0).getTel();

		// 회원수정 여부 물어보기
		Alert alert = new Alert(AlertType.CONFIRMATION);

		alert.setContentText("정말 수정 하시겠습니까???");
		alert.setTitle("알림");
		...
		if (bresult.get() == ButtonType.OK) {
			...
			try {
				...
				Parent parent = FXMLLoader.load(getClass().getResource("listupdate.fxml"));
